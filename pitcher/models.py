...
class Pitcher(models.Model):
    ...
    抢票进程
    '''
    pitcherName = models.CharField(u'进程名', max_length=100)
    ...
    class Meta:
        verbose_name = "抢票进程"
        verbose_name_plural = "(02)抢票进程"
        ordering = ['pitcherName']

    def __unicode__(self):
        return u'%s(%s)' % (self.pitcherName, self.username)
...
class SystemLog(models.Model):
    ...
    系统日志
    '''
    pitcher = models.ForeignKey(Pitcher, verbose_name=u'抢票进程', null=True, blank=True, default=None)
    logTime = models.DateTimeField(u"日志时间", default=datetime.now)
    logMsg = models.CharField(u'消息', max_length=500)

    class Meta:
        verbose_name = "系统日志"
        verbose_name_plural = "(05)系统日志"
...
class PitchConfig(models.Model):
    ...
    class Meta:
        verbose_name = "抢票配置"
        verbose_name_plural = "(03)抢票配置"
        ordering = ['pitcher','priority', '-need']
